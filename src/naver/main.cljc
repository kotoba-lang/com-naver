(ns naver.main
  "Kotodama WASM entrypoint for the Naver clean-room actor (L4) — Clojure port.
  ...
  facts (`naver.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Order"     :plural "orders"     :id-prefix "naver_ord"
   ...
   {:entity "Tracking"  :plural "trackings"  :id-prefix "naver_tra"
...
(defn emit-facts
  "EAVT facts for one record: {\"naver.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "naver-compat" :tier tier :entities entities} 200])
